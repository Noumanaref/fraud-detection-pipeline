...
parsed_tx_df = raw_tx_df.withColumn(
    "data", from_json(col("raw_payload"), mockingbird_schema)
).select("data.*")
parsed_legacy_df = legacy_df.withColumn(
    "data", from_json(col("raw_payload"), paysim_schema)
).select("data.*")


# lets standardize schemas one by one
std_tx_df = (
    parsed_tx_df.withColumnRenamed("user_id", "customer_id")
...
std_legacy_df = (
    parsed_legacy_df.withColumnRenamed("nameOrig", "customer_id")
    .withColumnRenamed("nameDest", "merchant_id")
    .withColumn(
        "transaction_id",
        sha2(
            concat_ws(
                "_", col("customer_id"), col("merchant_id"), col("amount"), col("step")
            ),
            256,
        ),
    )
    .withColumn(
        "timestamp", expr("timestamp('2026-07-01 00:00:00') + interval 1 hour * step")
    )
    .drop("step")
)
...
dim_time = (
    ...
    .withColumn(
        "is_weekend", when(dayofweek("timestamp").isin(1, 7), True).otherwise(False)
    )
...
dim_merchant = (
    silver_df.select("merchant_id", "transaction_type")
    .distinct()
...
fact_fraud_inference = (
    silver_df.withColumn("user_id", col("customer_id"))
    .withColumn("merchant_id", sha2(col("merchant_id"), 256))
    .withColumn("time_id", sha2(col("timestamp").cast("string"), 256))
...
if DeltaTable.isDeltaTable(spark, TIME_PATH):
    ...
    delta_time.alias("target").merge(
        dim_time.alias("source"), "target.time_id = source.time_id"
    ).whenNotMatchedInsertAll().execute()
else:
...
if DeltaTable.isDeltaTable(spark, MERCHANT_PATH):
    ...
    delta_merchant.alias("target").merge(
        dim_merchant.alias("source"), "target.merchant_id = source.merchant_id"
    ).whenMatchedUpdate(
        set={
