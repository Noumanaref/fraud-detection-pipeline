...
producer = KafkaProducer(
    bootstrap_servers=[
        "localhost:9093"
    ],  ## this bootstrap_server will tell the producer where to find the broker.
    # api_version = (2,6,0),
    value_serializer=lambda v: json.dumps(v).encode(
        "utf-8"
    ),  ## automatically converts python dictionaries to JSON bytes before sending.
)
...
try:
    while True:
        ...
        print(
            f"Sent ID: {transaction['transaction_id'][:8]}... | Amount: ${transaction['amount']}"
        )
