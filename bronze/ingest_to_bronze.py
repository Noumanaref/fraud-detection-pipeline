...
bronze_df = (
    raw_kafka_df.select(
        col("topic"), col("value").cast("string").alias("raw_payload"), col("timestamp")
    )
...
query1 = (
    raw_transactions_df.writeStream.outputMode("append")
    .format("parquet")
...
query2 = (
    legacy_batch_df.writeStream.outputMode("append")
    .format("parquet")
